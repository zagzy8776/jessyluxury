...
import {
  ...
  Star, RotateCcw, BadgeCheck, Sparkles, PackageSearch,
} from 'lucide-react'
import Bottle from '@/components/Bottle'
import ProductCard from '@/components/ProductCard'
import { useCart } from '@/components/CartProvider'
import { wa } from '@/lib/site'
import { formatNaira, products as fallbackProducts } from '@/lib/products'
    ...
    <span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          key={i}
          size={size}
          className={i <= Math.round(value) ? 'fill-[var(--champagne)] text-[var(--champagne)]' : 'text-[var(--border)]'}
        />
      ))}
    </span>
  )
...
export default function ProductDetailPage() {
  ...
  function useFallback() {
    ...
    if (fb) {
      ...
      setRelated(
        fallbackProducts
          .filter((p) => p.id !== fb.id && p.category === fb.category)
          .slice(0, 4)
      )
    } else {
      setError(true)
    ...
    return (
      <main className="flex min-h-[60vh] items-center justify-center bg-[var(--bg-primary)] px-6 text-center">
        <div>
          ...
          <p className="mt-1 text-xs text-[var(--text-muted)]">It may have sold out or been moved.</p>
    ...
    <main className="bg-[var(--bg-primary)] pb-28 text-[var(--text-primary)] lg:pb-0">
      <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6 lg:px-8">
        ...
        <nav aria-label="Breadcrumb" className="mb-6 flex items-center gap-1.5 text-[11px] font-semibold text-[var(--text-muted)]">
          <Link href="/shop" className="inline-flex items-center gap-1.5 transition hover:text-[var(--accent)]">
            <ArrowLeft size={14} /> Shop
          </Link>
          <span>/</span>
          <span className="truncate text-[var(--text-secondary)]">{product.name}</span>
        </nav>
        ...
        <div className="grid items-start gap-10 lg:grid-cols-2 lg:gap-14">
          ...
          <div>
            <div className="relative aspect-square overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--card-bg)] shadow-card">
              ...
              <button
                ...
                className="absolute right-5 top-5 z-10 rounded-full border border-[var(--border)] bg-white/90 p-3 shadow-sm backdrop-blur transition hover:scale-110"
              >
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={`h-20 w-20 shrink-0 overflow-hidden rounded-xl border-2 transition ${
                      activeImage === i ? 'border-[var(--accent)]' : 'border-[var(--border)] opacity-70 hover:opacity-100'
                    }`}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div className="space-y-7">
            <div>
              ...
              {avgRating != null && (
                <p className="mt-3 flex items-center gap-2 text-xs text-[var(--text-muted)]">
                  ...
                  <span className="font-bold text-[var(--text-secondary)]">{avgRating.toFixed(1)}</span>
              ...
              <div className="mt-5 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className={`font-display text-4xl font-bold tabular-nums ${hasSale ? 'text-[#b3372f]' : 'text-[var(--accent)]'}`}>
                  {formatNaira(price)}
                </span>
                {hasSale && (
                  <>
                    <span className="text-base tabular-nums text-[var(--text-muted)] line-through">
                      {formatNaira(product.price)}
                    </span>
                    <span className="rounded-full bg-[var(--champagne-soft)] px-2.5 py-1 text-[10px] font-bold tracking-[0.08em] text-[#7a5c22]">
              ...
              <p className="mt-3">
                {outOfStock ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-[#b3372f]/30 bg-[#b3372f]/10 px-3 py-1 text-[11px] font-bold text-[#b3372f]">
                    Out of stock
                  </span>
                ) : product.stock <= 5 ? (
            ...
            {product.description && (
              <p className="border-t border-[var(--border)] pt-5 text-sm leading-7 text-[var(--text-secondary)]">
                {product.description}
              </p>
            )}
            ...
            <div id="purchase-block" className="scroll-mt-24 space-y-3 border-t border-[var(--border)] pt-6">
              <div className="flex items-stretch gap-3">
                <div className="flex items-center rounded-full border border-[var(--border)] bg-[var(--card-bg)] px-1.5 shadow-card">
                  <button
                    ...
                    className="p-2.5 text-[var(--text-secondary)] transition hover:text-[var(--text-primary)] disabled:opacity-30"
                    disabled={qty <= 1}
              ...
              <a
                href={wa(`Hello Jessy Luxury! I want to order ${product.name} (${product.brand}, ${product.volume}) x${qty}. Price: ${formatNaira(price * qty)}`)}
                target="_blank"
                rel="noreferrer"
                className="flex w-full items-center justify-center gap-2 rounded-full border border-emerald-600/40 bg-emerald-50 py-3 text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-700 transition hover:bg-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-300"
              >
            ...
            <div className="grid grid-cols-1 gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-5 shadow-card sm:grid-cols-2">
              {[
                ...
                <div key={f.t} className="flex items-start gap-3">
                  <f.icon size={17} className="mt-0.5 shrink-0 text-[var(--accent)]" />
                  <div>
                    <p className="text-xs font-bold text-[var(--text-primary)]">{f.t}</p>
                    <p className="mt-0.5 text-[11px] leading-4 text-[var(--text-muted)]">{f.d}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {(noteRows.length > 0 || product.notes) && (
              <div className="rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-5 shadow-card">
                <h3 className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.2em] text-[var(--accent)]">
                  <Sparkles size={14} /> Fragrance Notes
                </h3>
                      ...
                      <div key={r.label} className="flex items-start gap-3">
                        <span
                          className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[9px] font-bold ${
                            ['bg-[var(--accent)] text-white', 'bg-[var(--accent)]/15 text-[var(--accent)]', 'bg-[var(--champagne-soft)] text-[#7a5c22]'][i]
                          }`}
                        >
                          {i + 1}
                        </span>
                        <div>
                          ...
                          <p className="text-sm font-medium text-[var(--text-primary)]">{r.value}</p>
                        </div>
                      </div>
                    ))
                  ) : (
          ...
          <div className="grid gap-10 lg:grid-cols-2">
            <div>
              ...
              {avgRating != null && (
                <div className="mt-4 flex items-center gap-3">
                  <span className="font-display text-4xl font-bold">{avgRating.toFixed(1)}</span>
                  <div>
                    <RatingStars value={avgRating} size={15} />
                    <p className="mt-0.5 text-xs text-[var(--text-muted)]">{reviews.length} review{reviews.length === 1 ? '' : 's'}</p>
                  </div>
                </div>
              )}
              ...
              <div className="mt-6 space-y-4">
                {reviews.length > 0 ? (
                  reviews.map((rev: any) => (
                    <div key={rev.id} className="rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] p-5 shadow-card">
                      <div className="flex items-center justify-between gap-3">
                        <span className="text-sm font-bold text-[var(--text-primary)]">{rev.customerName}</span>
                        <RatingStars value={rev.rating} />
                      </div>
                      <p className="mt-2 text-xs leading-6 text-[var(--text-secondary)]">&ldquo;{rev.comment}&rdquo;</p>
                    </div>
                  ))
                ) : (
                  <div className="rounded-2xl border border-dashed border-[var(--border)] p-8 text-center">
                    <p className="font-display text-lg font-bold text-[var(--text-primary)]">No reviews yet</p>
                    <p className="mt-1 text-xs text-[var(--text-muted)]">Be the first to share how this scent wears.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="h-fit rounded-3xl border border-[var(--border)] bg-[var(--card-bg)] p-6 shadow-card sm:p-8">
              ...
              <form onSubmit={handleReviewSubmit} className="mt-5 space-y-4 text-xs">
                <div>
                  <label className="mb-1.5 block font-bold text-[var(--text-secondary)]">Your name</label>
                  <input
                    value={revName}
                ...
                <div>
                  <label className="mb-1.5 block font-bold text-[var(--text-secondary)]">Rating</label>
                  <select
                    value={revRating}
                    onChange={(e) => setRevRating(Number(e.target.value))}
                    className="field-input font-bold text-[var(--champagne)]"
                  >
                ...
                <div>
                  <label className="mb-1.5 block font-bold text-[var(--text-secondary)]">Your review</label>
                  <textarea
                    value={revComment}
                ...
                <button type="submit" disabled={submittingRev} className="btn-primary w-full disabled:opacity-50">
          ...
          <section className="mt-16 border-t border-[var(--border)] pt-12 sm:mt-20">
            <div className="mb-8 flex items-end justify-between">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl font-bold">Complete the Collection</h2>
              </div>
              <Link href="/shop" className="hidden shrink-0 items-center gap-1.5 text-xs font-bold uppercase tracking-[0.14em] text-[var(--accent)] transition hover:text-[var(--accent-strong)] sm:flex">
                View all →
              </Link>
            </div>
      ...
      {showStickyBar && !outOfStock && (
        <div
          className="fixed inset-x-0 z-40 border-t border-[var(--border)] bg-[var(--card-bg)]/95 px-4 py-3 shadow-[0_-8px_24px_-12px_rgba(28,25,23,0.25)] backdrop-blur-md lg:hidden"
          style={{ bottom: 'calc(4.25rem + env(safe-area-inset-bottom))' }}
        >
          <div className="mx-auto flex max-w-lg items-center gap-3">
            <div className="min-w-0">
              <p className="truncate text-[11px] font-semibold text-[var(--text-muted)]">{product.name}</p>
              <p className="text-base font-bold tabular-nums text-[var(--accent)]">{formatNaira(price * qty)}</p>
            </div>
            <button
              onClick={() => {
                add(product, qty)
              ...
              className="btn-primary ml-auto shrink-0 !px-6 !py-3"
            >
              Add to Cart
            </button>
          </div>
