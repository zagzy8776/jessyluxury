    ...
    <main className="bg-[var(--bg-primary)] text-[var(--text-primary)]">
      <section className="relative overflow-hidden border-b border-[var(--border)] bg-[var(--card-bg)]">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 text-center lg:px-8 lg:py-20">
          <p className="text-[10px] font-bold tracking-[0.26em] text-amber-500">THE WORLD OF JESSY</p>
          <h1 className="mt-3 font-display text-5xl font-bold text-[var(--text-primary)] sm:text-6xl">Gallery</h1>
      ...
      <section className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {tiles.map((t, i) => (
            <div
              key={t.label}
              className={`group relative flex aspect-[4/5] flex-col items-center justify-center overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--card-bg)] transition hover:border-amber-500 shadow-sm ${
                i % 3 === 1 ? 'md:translate-y-6' : ''
              }`}
            >
              <div className="grain absolute inset-0 opacity-30" />
              <Bottle tone={t.tone} className="scale-90 transition duration-700 group-hover:scale-100" />
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[var(--card-bg)] to-transparent p-4">
                <p className="font-display text-lg font-bold text-[var(--text-primary)]">{t.label}</p>
                <p className="text-[10px] tracking-[0.14em] text-[var(--text-muted)] font-bold">{t.note}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 flex items-center justify-center">
          <div className="relative flex h-64 w-full max-w-3xl items-center justify-center overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--card-bg)] shadow-sm">
            <div className="grain absolute inset-0 opacity-30" />
            <div className="absolute h-56 w-56 rounded-full bg-amber-500/10 blur-3xl" />
            <img src="/logo.png.jpeg" alt="Jessy Luxury" className="relative h-32 w-auto object-contain drop-shadow-xl" />
          </div>
        </div>
      </section>
    </main>
